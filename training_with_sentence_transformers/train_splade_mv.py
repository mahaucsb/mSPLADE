...
parser.add_argument("--token_sparse", default=False, action="store_true")
parser.add_argument("--gumbel", default=False, action="store_true")
parser.add_argument("--logp_3v", default=0.0, type=float)
...
word_embedding_model = models.MVMLMTransformer(model_name, max_seq_length=max_seq_length, token_sparse=args.token_sparse, gumbel = args.gumbel)

## initialize the values for the MLM head:
if model_name == "Luyu/co-condenser-marco":
    x = torch.load("../../msmarco/cocondenser.bin",map_location=torch.device('cpu'))
    with torch.no_grad():
        ...
        word_embedding_model.cls.module.decoder.weight = torch.nn.Parameter(x['cls.predictions.decoder.weight'].repeat(3,1) +  x['cls.predictions.decoder.weight'].abs().mean() * torch.rand(x['cls.predictions.decoder.weight'].repeat(3,1).shape).to(x['cls.predictions.decoder.weight'].device))
        word_embedding_model.cls.module.decoder.bias = torch.nn.Parameter(x['cls.predictions.decoder.bias'].repeat(3) +  x['cls.predictions.decoder.bias'].abs().mean() * torch.rand(x['cls.predictions.decoder.bias'].repeat(3).shape).to(x['cls.predictions.decoder.bias'].device))

model = SentenceTransformerA(modules=[word_embedding_model])

model_save_path = f'output/splade_3V_distill_tokensparse{args.token_sparse}-gumbel{args.gumbel}_{args.loss_type}_denoise{args.denoise}_num{args.num_negs_per_system}_{args.loss_type}-lambda{args.lambda_q}-{args.lambda_d}_lr{args.lr}-batch_size_{train_batch_size}x{args.accum_iter}-{datetime.now().strftime("%Y-%m-%d")}'
...
if args.loss_type == 'marginmse_ib':
    ...
    train_loss = losses.MarginMSELossSplade(model=model, lambda_d=args.lambda_d, lambda_q=args.lambda_q, logp_3v = args.logp_3v)
    ...
    train_loss = losses.KLAddRegLossSplade(model=model, beta_p = args.beta_p, beta_n = args.beta_n)
