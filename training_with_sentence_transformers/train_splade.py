...
parser.add_argument("--sample_upweight", default = 0.0, type=float, help="For kldiv + added penaulty (klar), sample upweight != 0 will be used to upweight samples where query do good.")
...
if args.loss_type in ["kldiv_focal","kldiv_position_focal", "kldiv_multipos_focal", "kldiv_multipos_position_focal"]: 
    model_save_path = f'output/splade_distill_num1_{args.loss_type}_{args.weight_option}_gamma{args.gamma}_denoise{args.denoise}_num{args.num_negs_per_system}_{args.loss_type}{args.nway}-lambda{args.lambda_q}-{args.lambda_d}_lr{args.lr}-batch_size_{train_batch_size}x{args.accum_iter}-{datetime.now().strftime("%Y-%m-%d")}'
elif args.loss_type in ["kldiv_position_focal"]: 
    ...
    model_save_path = f'output/splade_distill_num1_{args.loss_type}-{args.beta_p}-{args.sample_upweight}_denoise{args.denoise}_num{args.num_negs_per_system}_{args.nway}-lr{args.lr}-batch_size_{train_batch_size}x{args.accum_iter}-{datetime.now().strftime("%Y-%m-%d")}'
elif args.loss_type == 'kldiv_ib':
    model_save_path = f'output/splade_distill_num1_{args.loss_type}_ib{args.ib_p}_denoise{args.denoise}_num{args.num_negs_per_system}_{args.loss_type}{args.nway}-lr{args.lr}-batch_size_{train_batch_size}x{args.accum_iter}-{datetime.now().strftime("%Y-%m-%d")}'
elif args.loss_type in  ["kldiv_multipos", "kldiv"]:
    model_save_path = f'output/splade_distill_num1_{args.loss_type}-upw{args.sample_upweight}_denoise{args.denoise}_num{args.num_negs_per_system}_{args.loss_type}{args.nway}-lr{args.lr}-batch_size_{train_batch_size}x{args.accum_iter}-{datetime.now().strftime("%Y-%m-%d")}'
else:
    model_save_path = f'output/splade_distill_num1_{args.loss_type}_{args.weight_option}_denoise{args.denoise}_num{args.num_negs_per_system}_{args.loss_type}{args.nway}-lambda{args.lambda_q}-{args.lambda_d}_lr{args.lr}-batch_size_{train_batch_size}x{args.accum_iter}-{datetime.now().strftime("%Y-%m-%d")}'

#model_save_path = f'output/distilSplade_fromcocondenser_{args.lambda_q}_{args.lambda_d}_{model_name.replace("/", "-")}-batch_size_{train_batch_size}-{datetime.now().strftime("%Y-%m-%d_%H-%M-%S")}'
...
if args.loss_type == 'marginmse_ib':
    ...
    train_loss = losses.KLDivLossSplade(model=model,  lambda_d=args.lambda_d, lambda_q=args.lambda_q, sample_upweight = args.sample_upweight)
elif args.loss_type == "kldiv_multipos":
    train_loss = losses.KLDivLossSplade(model=model,  lambda_d=args.lambda_d, lambda_q=args.lambda_q, multipos=True, sample_upweight = args.sample_upweight)
    ...
    train_loss = losses.KLDivLossSpladeInBatch(model=model, lambda_d=args.lambda_d, lambda_q=args.lambda_q, inbatch_p = args.ib_p)
elif args.loss_type == 'curriculum':
    train_loss = losses.CRLossSplade(model=model, lambda_d=args.lambda_d, lambda_q=args.lambda_q)
elif args.loss_type == "klar":
    train_loss = losses.KLAddRegLossSplade(model=model, beta_p = args.beta_p, sample_upweight = args.sample_upweight)
elif args.loss_type == "kllog":
    train_loss = losses.KLLogRegLossSplade(model=model, beta_p = args.beta_p, sample_upweight = args.sample_upweight)
