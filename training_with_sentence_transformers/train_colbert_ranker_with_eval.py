...
if args.loss_type in ["kldiv_focal", "kldiv_position_focal"]: 
    model_save_path = f'output/colbert_splade_distill_num1_{args.loss_type}_{args.weight_option}_gamma{args.gamma}-alpha{args.alpha}_denoise{args.denoise}_num{args.num_negs_per_system}_{args.loss_type}{args.nway}-lr{args.lr}-batch_size_{train_batch_size}x{args.accum_iter}-{datetime.now().strftime("%Y-%m-%d")}'
elif args.loss_type == 'kldiv_ib':
    model_save_path = f'output/colbert_splade_distill_num1_{args.loss_type}_ib{args.ib_p}_denoise{args.denoise}_num{args.num_negs_per_system}_{args.loss_type}{args.nway}-lr{args.lr}-batch_size_{train_batch_size}x{args.accum_iter}-{datetime.now().strftime("%Y-%m-%d")}'
elif args.loss_type == 'klar':
    model_save_path = f'output/colbert_splade_distill_num1_{args.loss_type}-{args.beta_p}-{args.beta_n}_denoise{args.denoise}_num{args.num_negs_per_system}_{args.nway}-lr{args.lr}-batch_size_{train_batch_size}x{args.accum_iter}-{datetime.now().strftime("%Y-%m-%d")}'
elif args.loss_type == "kldiv_multipos_focal":
    model_save_path = f'output/colbert_splade_distill_num1_fixed_beta_{args.loss_type}_{args.weight_option}_gamma{args.gamma}-alpha{args.alpha}-alpha{args.beta}_denoise{args.denoise}_num{args.num_negs_per_system}_{args.loss_type}{args.nway}-lr{args.lr}-batch_size_{train_batch_size}x{args.accum_iter}-{datetime.now().strftime("%Y-%m-%d")}'
else:
    model_save_path = f'output/colbert_splade_distill_num1_{args.loss_type}_{args.weight_option}_alpha{args.alpha}_denoise{args.denoise}_num{args.num_negs_per_system}_{args.loss_type}{args.nway}-lr{args.lr}-batch_size_{train_batch_size}x{args.accum_iter}-{datetime.now().strftime("%Y-%m-%d")}'
...
if args.loss_type == "marginmse_position":
    ...
    train_loss = losses.KLDivLossColBERT(model=model, multipos = True)
elif args.loss_type == 'kldiv_ib':
    train_loss = losses.KLDivLossColBERTInBatch(model=model, inbatch_p = args.ib_p)
elif args.loss_type == "kldiv_position":
    train_loss = losses.KLDivLossColBERT(model=model, scaled = True, alpha=args.alpha,weight_option=args.weight_option)
    ...
    train_loss = losses.KLDivLossColBERT(model=model, scaled = True, focal=True, gamma = args.gamma, alpha = args.alpha, multipos = True)
...
elif args.loss_type == 'marginkldiv_position':
    train_loss = losses.MarginKLDivLossColBERT(model=model, scaled = True, alpha=args.alpha,weight_option=args.weight_option)
elif args.loss_type == 'wce':
    train_loss = losses.MultipleNegativesRankingLossColBERT(model=model, scaled = True, alpha=args.alpha, weight_option=args.weight_option)
    ...
    train_loss = losses.KLAddRegLossColBERT(model=model, beta_p = args.beta_p, beta_n = args.beta_n)
else:
    raise "Unknown loss!"
