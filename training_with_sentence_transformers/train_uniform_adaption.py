...
parser.add_argument("--epochs", default=40, type=int)#150k == 38.17...
parser.add_argument("--negs_to_use", default=None, help="From which systems should negatives be used ? Multiple systems seperated by comma. None = all")
parser.add_argument("--warmup_steps", default=6000, type=int)
parser.add_argument("--lambda_uni", default=0.01, type=float)
...
query_layer = smodel.Dense(in_features=dim, out_features = dim, activation_function = nn.Identity())

model = SentenceTransformer(modules=[word_embedding_model, query_layer])
model_save_path = f'output/distilSplade_lastquerylayer_lambdauni-{args.lambda_uni}_batch_size_{train_batch_size}-{datetime.now().strftime("%Y-%m-%d_%H-%M-%S")}'
...
train_loss = losses.MarginMSELossSpladeAdapt(model=model, lambda_uni = args.lambda_uni)
