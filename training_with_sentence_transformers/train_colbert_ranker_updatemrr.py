...
word_embedding_model = models.ColBERTTransformer(model_name, max_seq_length=max_seq_length,dim=128)
...
#model_save_path = f'output/colbert_splade_distill_num1_updatemrr{args.mrr_update}_{args.loss_type}_gamma{args.gamma}-alpha{args.alpha}_denoise{args.denoise}_num{args.num_negs_per_system}_{args.loss_type}{args.nway}-lr{args.lr}-batch_size_{train_batch_size}x{args.accum_iter}-{datetime.now().strftime("%Y-%m-%d")}'
model_save_path = f'output/colbert128_splade_distill_num1_updatemrr{args.mrr_update}_{args.loss_type}_gamma{args.gamma}-alpha{args.alpha}-ibp{args.ib_p}_denoise{args.denoise}_num{args.num_negs_per_system}_{args.loss_type}{args.nway}-lr{args.lr}-batch_size_{train_batch_size}x{args.accum_iter}-{datetime.now().strftime("%Y-%m-%d")}'
...
if args.loss_type == "kldiv_multipos_position_focal":
    train_loss = losses.KLDivLossColBERT(model=model, scaled = True, focal=True, gamma = args.gamma, alpha = args.alpha, multipos = True)
elif args.loss_type == "kldiv_ib_position_focal":
    train_loss = losses.KLDivLossColBERT(model=model, scaled = True, focal=True, gamma = args.gamma, alpha = args.alpha, multipos = False, ib_p = args.ib_p)
elif args.loss_type == "kldiv_multipos_ib_position_focal":
    train_loss = losses.KLDivLossColBERT(model=model, scaled = True, focal=True, gamma = args.gamma, alpha = args.alpha, multipos = True, ib_p = args.ib_p)
else:
    raise "Unknown loss!"
...
for iter in range(int(num_epochs / update_n)):
    ...
    train_dataset = MSMARCODataset(queries=train_queries_sub, corpus=corpus, ce_scores=ce_scores, loss_type=args.loss_type, num_neg=args.nway, topk = 20, reeval = True, curmodel = model, tokenizer = tokenizer)
    train_dataloader = DataLoader(train_dataset, shuffle=True, batch_size=train_batch_size, drop_last=True)
