...
from cq_models import Code_Learner
import gzip
#import nvidia_smi
...
dense_weight = 0.0
model = ColBERTTransformer(model_type_or_dir, max_seq_length=256, dim=768)
checkpoint = torch.load(os.path.join(model_type_or_dir, "checkpoint.pt"), map_location='cpu')
model.load_state_dict(checkpoint)

code_learner = Code_Learner(emb_size=768, M=96, K=256)
    ...
    if cur_qid is not None and qid != cur_qid:
        with torch.no_grad():
            ...
            for batch in d_features:
                ...
                d_features = d_features.reshape(token_rep_d.shape[0], token_rep_d.shape[1], -1)
                
                
                token_rep_d = torch.nn.functional.normalize(d_features, p=2, dim=1)
                #token_rep_d = torch.nn.functional.normalize(token_rep_d, p=2, dim=1)
