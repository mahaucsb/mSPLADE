...
for idx, line in enumerate(tqdm(topk)):
    ...
    if (idx+1) % num == 0:
        with torch.no_grad():
            d_features = tokenizer(["[unused0] " + qtext + " [SEP] [unused1] " + dtext for qtext, dtext in zip(qtexts, dtexts)], return_tensors="pt", max_length=256,truncation=True,padding=True)
            d_features = _split_into_batches(d_features,bsize=bsize)
            
            #if qtext_prf is not None:
            #    d_features_prf = tokenizer(["[unused0] " + qtext_prf + " [SEP] [unused1] " + dtext for qtext_prf, dtext in zip(qtexts_prf, dtexts)], return_tensors="pt", max_length=256,truncation=True,padding=True)
            #    d_features_prf = _split_into_batches(d_features_prf,bsize=bsize)
            
            all_scores = []
            #all_scores_prf = []
            for batch in d_features:
                out = model(batch)
                scores = out['cls'].to('cpu')
                del out
