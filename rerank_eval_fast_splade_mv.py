...
import sys
import numpy as np
from training_with_sentence_transformers.models import MVMLMPredictionHead
...
class Splade(torch.nn.Module):
    ...
    def __init__(self, model_type_or_dir, agg="max", token_sparse = False):
        super().__init__()
        self.bert = BertModel.from_pretrained(model_type_or_dir)
        self.config = AutoConfig.from_pretrained(model_type_or_dir)
        self.cls = MVMLMPredictionHead(self.config)
    ...
    def forward(self, **kwargs):
        output_states = self.bert(**kwargs) # output (logits) of MLM head, shape (bs, pad_len, voc_size)
        ...
        if self.agg == "max":
            values, _ = torch.max(torch.log(1 + torch.relu(prediction_scores)) * kwargs["attention_mask"].unsqueeze(-1), dim=1)
            return values
            # 0 masking also works with max because all activations are positive
        else:
            return torch.sum(torch.log(1 + torch.relu(prediction_scores)) * kwargs["attention_mask"].unsqueeze(-1), dim=1)

model = Splade(model_type_or_dir, agg=agg, token_sparse=False)
checkpoint = torch.load(os.path.join(model_type_or_dir, "checkpoint.pt"), map_location='cpu')
rename = dict()
for k in checkpoint:
    rename[k.replace("module.", "")] = checkpoint[k]
model.load_state_dict(rename)
del checkpoint    
print("model loaded")
model.eval()
model.to("cuda")
