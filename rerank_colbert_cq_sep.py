...
from cq_models_sep import Encoder, Decoder, Reader, Coder
import gzip
#import nvidia_smi
...
model.to('cuda')
#code_learner = Code_Learner(emb_size=int(sys.argv[7]), M=16, K=256)
#code_learner.load_state_dict(torch.load(cq_model_dir))

epoch_num = sys.argv[8]
encoder = Encoder(emb_size = int(sys.argv[7]), hidden_size = None)
coder = Coder(M = 16, K = 256, hidden_size = 128)
reader = Reader(M = 16, K = 256, output_size = 128)
decoder = Decoder(output_size = 128, emb_size = int(sys.argv[7]))

encoder.load_state_dict(torch.load(cq_model_dir.replace("num",epoch_num).replace("type", "encoder")))
coder.load_state_dict(torch.load(cq_model_dir.replace("num",epoch_num).replace("type", "coder")))
reader.load_state_dict(torch.load(cq_model_dir.replace("num",epoch_num).replace("type", "reader")))
decoder.load_state_dict(torch.load(cq_model_dir.replace("num",epoch_num).replace("type", "decoder")))
    ...
    if cur_qid is not None and qid != cur_qid:
        with torch.no_grad():
            ...
            for batch in d_features:
                ...
                #d_features = code_learner(cq_input, training = False)
                residual = models['encoder'](cq_input)
                codes, _ = models['coder'](residual, training=False)
                approx_residual = models['reader'](codes)
                d_emb = models['decoder'](approx_residual, cq_input[:,int(sys.argv[7]):])

                d_emb = d_emb.reshape(token_rep_d.shape[0], token_rep_d.shape[1], -1)
               
                token_rep_d = torch.nn.functional.normalize(d_emb, p=2, dim=2)
